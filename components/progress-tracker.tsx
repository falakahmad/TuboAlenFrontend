...
export default function ProgressTracker({
  ...
  const renderSparkline = (data: number[], color: string, label: string) => {
    // Show sparkline even for single data point (Pass 1)
    ...
    // For single data point, show just the point (no line)
    let points: string | null = null
    if (data.length === 1) {
      // Single point - don't draw a line, just show the point
      points = null
    } else {
      points = data
        .map((value, index) => {
    ...
    return (
      <div className="space-y-1">
        ...
        <svg width="80" height="20" className="overflow-visible">
          ...
          {data.map((_, index) => {
            const cx = data.length === 1 ? 40 : (index / (data.length - 1)) * 80
            const cy = data.length === 1 ? 10 : 20 - ((data[index] - min) / range) * 20
            return (
              <circle
                key={index}
                cx={cx}
      ...
      <CardHeader>
        <div className="flex items-center justify-between gap-4">
          <div className="min-w-0 flex-1">
            <CardTitle className="text-card-foreground text-lg break-words">{fileName}</CardTitle>
      ...
      <CardContent className="space-y-6">
        {passData.map((pass) => (
          <div key={pass.passNumber} className="space-y-3">
            ...
            {/* Stage Chips */}
            <div className="flex flex-wrap gap-2 w-full p-3 bg-muted/50 rounded-lg border border-border">
              {pass.stages.map((stage) => (
                <div 
                  key={stage.name} 
                  className={`px-3 py-1.5 rounded-md text-xs border ${getStageColor(stage.status)}`}
                  style={{ minWidth: 'fit-content' }}
                >
                  <div className="flex items-center gap-1.5 whitespace-nowrap">
                    <span>{stage.name}</span>
                    {stage.status === "running" && <div className="w-2 h-2 bg-blue-400 rounded-full animate-pulse flex-shrink-0" />}
                    {stage.duration && (
            ...
            {/* Sparklines */}
            {pass.sparklineData && (
              <div className="grid grid-cols-4 gap-4 p-3 bg-muted rounded-lg border border-border w-full">
                <div
