...
export async function POST(request: NextRequest) {
  try {
    ...
    if (backendUrl) {
        ...
        if (token) {
          ...
          if (!usageUserId && decoded?.email) {
            try {
              ...
              if (mod?.supabase) {
                const { data } = await mod.supabase.from('users').select('id').eq('email', String(decoded.email).toLowerCase()).single()
                if (data?.id) usageUserId = data.id
              }
            } catch {}
          }
        }
      ...
      // Increase timeout to allow long-running SSE streams
      ...
      try {
        ...
        return NextResponse.json({ error: "Backend fetch timeout or connection refused" }, { status: 504 })
      } finally {
        clearTimeout(timeout)
      }
      ...
      try {
        const mod = require('@/lib/supabase')
        if (mod?.supabase) {
          await mod.supabase.from('system_logs').insert({ user_id: usageUserId, action: 'refine_start', details: `files=${(body.files||[]).length}, passes=${body.passes}` })
        }
      } catch {}
      ...
      const stream = new ReadableStream({
        async start(controller) {
          ...
          try {
            ...
            const enc = new TextEncoder()
            controller.enqueue(enc.encode(`data: ${JSON.stringify({ type: "error", error: "proxy_stream_error" })}\n\n`))
            ...
            try {
              const mod = require('@/lib/supabase')
              if (mod?.supabase) {
                await mod.supabase.from('system_logs').insert({ user_id: usageUserId, action: sawTerminal ? 'refine_complete' : 'refine_stream_error', details: `chunks=${chunkCount}` })
                // Persist usage stats snapshot (user_id may be null if unknown)
                if (totalTokensInUsed > 0 || totalCost > 0 || totalRequests > 0) {
                  try {
                    if (usageUserId) {
                      await mod.supabase.rpc('update_usage_stats', {
                        p_user_id: usageUserId,
                        p_request_count: Math.max(1, totalRequests),
                        p_token_count: Math.max(0, Math.floor(totalTokensInUsed)),
                        p_cost: Math.max(0, Number(totalCost.toFixed(4)))
                      })
                    } else {
                      // Fallback when no user_id resolved: insert a row without user attribution
                      await mod.supabase.from('usage_stats').insert({
                        user_id: null,
                        request_count: Math.max(1, totalRequests),
                        token_count: Math.max(0, Math.floor(totalTokensInUsed)),
                        cost: Math.max(0, Number(totalCost.toFixed(4)))
                      })
                    }
                  } catch {}
                }
